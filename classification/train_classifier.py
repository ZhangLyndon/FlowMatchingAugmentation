...
class ClassificationTrainer:
	"""
	Trainer for fashion item classification with synthetic data augmentation.
	"""
	def __init__(self, args):
		self.args = args
		self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

		self.model = create_classifier(num_classes = 10).to(self.device)
...
def main():
	...
	train_loader, test_loader = get_dataloaders(root_dir = args.data_root,
